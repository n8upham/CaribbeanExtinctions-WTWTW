...
all.f$ID <- factor(all.f$ID, levels = c("mass:fhar", "fhar", "hurr", "elev" , "mas2", "mass", "(Intercept)"))
...
fix.plot<-ggplot(all.f,aes(ID, y=`mean`, ymin= `0.025`, ymax= `0.975`))+geom_hline(yintercept=0, linetype="dashed", colour="grey75", size=.75)+geom_pointrange(colour=all.f$col)+theme_pander()+ coord_flip()+ylab("Coefficient on probability of survival")+xlab("Sample-wide effects")+ scale_x_discrete(labels=c("mass:fhar"="Mass by first human arrival", "fhar"="First human arrival\n(Ka since present)", "elev"="Mean island elevation", "mass"="Mass", "mas2"= bquote(''*Mass^2*''), "(Intercept)"="Intercept"))
       
##save
ggsave("sample_miss_data_v3.pdf", h=5, w=5)
...
##save
ggsave("species_miss_data_v3.pdf", h=6, w=6)
...
##save
ggsave("islands_miss_data_v3.pdf", h=8, w=4)
...
##reorder so they plot right
all.f$ID <- factor(all.f$ID, levels = c("mass:fhar", "fhar", "elev" , "mas2", "mass", "(Intercept)"))

##create a new column with colors
all.f$col<-ifelse(all.f$'0.975'<0, "grey20", ifelse(all.f$'0.025'>0, "grey20", "grey80"))

##plot sample-wide effects
fix.plot<-ggplot(all.f,aes(ID, y=`mean`, ymin= `0.025`, ymax= `0.975`))+geom_hline(yintercept=0, linetype="dashed", colour="grey75", size=.75)+geom_pointrange(colour=all.f$col)+theme_pander()+ coord_flip()+ylab("Coefficient on probability of survival")+xlab("Sample-wide effects")+ scale_x_discrete(labels=c("mass:fhar"="Mass by first human arrival", "fhar"="First human arrival\n(Ka since present)", "mass"="Mass", "elev"="Mean island elevation", "mas2"= bquote(''*Mass^2*''), "(Intercept)"="Intercept"))
       
##save
ggsave("sample_impu_data_v2.pdf", h=5, w=5)
...
##save
ggsave("species_impu_data_v3.pdf", h=6, w=6)
...
##save
ggsave("islands_impu_data_v3.pdf", h=8, w=4)
...
##reorder so they plot right
##mongoose was excluded
all.f$ID <- factor(all.f$ID, levels = c("mass:fhar", "mong", "impa", "loss", "fore" , "fhar", "hurr", "volc", "elev" , "area", "mas2", "mass", "(Intercept)"))


##create a new column with colors
all.f$col<-ifelse(all.f$'0.975'<0, "grey20", ifelse(all.f$'0.025'>0, "grey20", "grey80"))

##deal with those massive credible intervals
##https://stackoverflow.com/questions/3606697/how-to-set-limits-for-axes-in-ggplot2-r-plots

##plot sample-wide effects
fix.plot<-ggplot(all.f,aes(ID, y=`mean`, ymin= `0.025`, ymax= `0.975`))+geom_hline(yintercept=0, linetype="dashed", colour="grey75", size=.75)+geom_pointrange(colour=all.f$col)+theme_pander()+ coord_flip(ylim = c(-6,6)) +ylab("Coefficient on probability of survival")+xlab("Sample-wide effects")+ scale_x_discrete(labels=c("mass:fhar"="Mass by first human arrival", "mong"="Mongoose present", "hurr"="Hurricane frequency", "volc"="Active volcano", "impa"="Human Footprint Index", "loss"="Forest loss\n(2000-2014)", "fore"="Forest cover (2000)", "fhar"="First human arrival\n(Ka since present)", "elev"="Mean island elevation", "area"=" Island area", "mas2"= bquote(''*Mass^2*''), "mass"="Mass", "(Intercept)"="Intercept"))
       
##save
ggsave("sample_full_data_v3.pdf", h=6, w=6)
...
##save
ggsave("species_full_data_v3.pdf", h=6, w=6)
...
##save
ggsave("islands_full_data_v3.pdf", h=8, w=4)
...
fhar_val <- mean(na.omit(dat1$fhar))
hurr_val <- mean(na.omit(dat1$hurr))
elev_val <- mean(na.omit(dat1$elev))
area_val <- mean(na.omit(dat1$area))
fore_val <- mean(na.omit(dat1$fore))
int1_val <- quantile(na.omit(dat1$mass*dat1$fhar), .25)
int2_val <- quantile(na.omit(dat1$mass*dat1$fhar), .5)
int3_val <- quantile(na.omit(dat1$mass*dat1$fhar), .75)

##make values
##low interaction
a_logits <- all.f['(Intercept)','mean'] + all.f['mass','mean'] * m_range + all.f['mas2','mean'] * m_range^2 + all.f['area','mean'] * area_val + all.f['elev','mean'] * elev_val + all.f['hurr','mean'] * hurr_val + all.f['fhar','mean'] * fhar_val + all.f['loss','mean'] * loss_val + all.f['fore','mean'] * fore_val + all.f['impa','mean'] * impa_val + all.f['mass:fhar','mean'] * int1_val

##medium interaction
b_logits <- all.f['(Intercept)','mean'] + all.f['mass','mean'] * m_range + all.f['mas2','mean'] * m_range^2 + all.f['area','mean'] * area_val + all.f['elev','mean'] * elev_val + all.f['hurr','mean'] * hurr_val + all.f['fhar','mean'] * fhar_val + all.f['loss','mean'] * loss_val + all.f['fore','mean'] * fore_val + all.f['impa','mean'] * impa_val + all.f['mass:fhar','mean'] * int2_val

##high interaction
c_logits <- all.f['(Intercept)','mean'] + all.f['mass','mean'] * m_range + all.f['mas2','mean'] * m_range^2 + all.f['area','mean'] * area_val + all.f['elev','mean'] * elev_val + all.f['hurr','mean'] * hurr_val + all.f['fhar','mean'] * fhar_val + all.f['loss','mean'] * loss_val + all.f['impa','mean'] * impa_val + all.f['fore','mean'] * fore_val + all.f['mass:fhar','mean'] * int3_val
...
plot.data <- data.frame(recent=a_probs, intermediate=b_probs, ancient=c_probs, mast=m_range)
plot.data <- gather(plot.data, key=group, value=prob, recent:intermediate:ancient)

##reorder so they plot right
plot.data$group <- factor(plot.data$group, levels = c("recent", "intermediate", "ancient"))

##print out the table to get probabilities
write.csv(plot.data, "all_predict_table_v3.csv")
...
m.plot<-ggplot(plot.data, aes(x=mass, y=prob, color=group)) + # asking it to set the color by the variable "group" is what makes it draw three different lines
  ...
  labs(x="Mass in Kg", y="Probability of survival")+theme_few()+scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
              labels = trans_format("log10", math_format(10^.x)))+scale_colour_grey()+geom_rug(data =as.data.frame(x), inherit.aes = F, aes(x=x))+ theme(legend.position="top") + labs(colour = "First human arrival")

##save
ggsave("mass_full_data_v3.pdf", h=4, w=6)
